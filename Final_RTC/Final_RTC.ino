...
// ============================================================
// RTC
constexpr int PIN_RTC_SDA = 8;
constexpr int PIN_RTC_SCL = 9;
// TFT / SD shared SPI
...
void initializeRtc() {
  ...
  if (rtc.lostPower()) {
    Serial.println("[WARN] RTC lost power - waiting for NTP sync");
  }
}
// ============================================================
...
bool synchronizeNetworkTime() {
  ...
  if (rtcAvailable) {
    ...
    Serial.println("[OK] RTC synchronized from NTP");
  }
...
bool readLocalTime(struct tm &timeInfo) {
  ...
  // 1. ESP32의 NTP/System 시간이 있으면 우선 사용
  if (getLocalTime(&timeInfo, 10)) {
    timeSynchronized = true;
    return true;
  }

  // 2. 인터넷 시간이 없으면 RTC 사용
