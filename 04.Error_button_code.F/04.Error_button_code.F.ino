...
unsigned long lastStopwatchPrintTime = 0;
...
void loop() {
  ...
  // 좌측+중앙 동시 입력을 가장 먼저 확인
  handleResetCombination();

  /*
   * 초기화 조합이 진행 중일 때는
   * 좌측·중앙 버튼의 개별 기능을 실행하지 않는다.
   */
...
void printStopwatch() {
  ...
  // 정지 상태에서는 같은 값을 반복 출력하지 않음
  if (!stopwatchRunning) {
    return;
  }

  // IDE 렉 방지를 위해 0.25초마다 출력
  if (millis() - lastStopwatchPrintTime < 250) {
    return;
  }

  lastStopwatchPrintTime = millis();
