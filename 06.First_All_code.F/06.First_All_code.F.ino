...
void updateLedForCurrentState() {
  if (currentScreen == ScreenMode::RECORDER) {
    ...
    if (
      indicatedPlaySlot >= 0 &&
      static_cast<int32_t>(playIndicatorUntil - millis()) > 0
    ) {
...
void updateButton(Button &button) {
  ...
  if (
    ...
    !button.longPressHandled &&
    now - button.pressedTime >= LONG_PRESS_MS
  ) {
...
void handleStopwatchResetCombination() {
  ...
  if (leftPressed && centerPressed) {
    ...
    if (
      !comboHandled &&
      millis() - comboStartedAt >= COMBO_HOLD_MS
    ) {
      comboHandled = true;
      resetStopwatch();
    }
...
void stopRecording(bool automaticStop) {
  ...
  recordingSlots[savedSlot].durationMs = duration;
...
void playNextRecording() {
  ...
  for (
    ...
    logEvent("Recording playback selected");

    screenContentDirty = true;
    updateLedForCurrentState();

    // 실제 WAV 재생 코드는 추후 이 위치에 연결
    return;
  }
}
...
void drawStopwatchContent() {
  ...
  drawCenteredText(
    formatStopwatchTime(getStopwatchTime()),
    tft.width() / 2,
    75,
    5,
    stopwatchRunning
...
void drawRecorderContent() {
  ...
  if (recording) {
    ...
    drawCenteredText(
      formatRecordingTime(getRecordingTime()),
      tft.width() / 2,
      92,
      4,
      ILI9341_WHITE
    ...
    drawCenteredText(
      formatRecordingTime(
        recordingSlots[indicatedPlaySlot].durationMs
      ),
      tft.width() / 2,
      95,
      3,
...
void updateScreen(bool force = false) {
  ...
  uint32_t updateInterval = CLOCK_UPDATE_MS;

  switch (currentScreen) {
    case ScreenMode::CLOCK:
      updateInterval = CLOCK_UPDATE_MS;
      break;
  ...
  if (
    !force &&
    !screenContentDirty &&
    millis() - lastScreenUpdate < updateInterval
  ) {
    return;
  }
...
void changeScreen() {
  ...
  currentScreen =
    static_cast<ScreenMode>(nextScreen);
...
void handleButtonEvents() {
  ...
  switch (currentScreen) {
    ...
    case ScreenMode::RECORDER:
      ...
      if (buttonCenter.shortPressEvent) {
        if (recording) {
          stopRecording(false);
        } else {
          playNextRecording();
